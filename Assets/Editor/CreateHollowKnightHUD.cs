...
public static class CreateHollowKnightHUD
{
    ...
    public static void AddOrUpdateSoulPanel()
    {
        ...
        // SoulPanel만 초기화(재생성)해서 RectTransform/클리핑 상태 누적 꼬임을 제거합니다.
        ...
        Debug.Log("[Hollow Knight HUD] SoulPanel을 재생성하고 atlas0_308 충전 프레임을 다시 할당했습니다.");
    }
    ...
    private static GameObject CreateSoulPanel(Transform parent)
    {
        ...
        // 원형 클리핑(Mask) 제거: RectMask2D(FillViewport)만으로 세로 클립만 유지합니다.
    ...
    private static void EnsureSoulPanelReferences(SoulUI soulUI, SerializedObject so)
    {
        ...
        // FillViewport가 이미 존재하더라도 RectTransform 높이가 0으로 꼬여 있을 수 있으므로
        // 항상 동일한 앵커/피벗/사이즈를 재적용합니다.
        if (fillViewportRect != null)
        {
            fillViewportRect.anchorMin = new Vector2(0, 0);
            fillViewportRect.anchorMax = new Vector2(1, 0);
        ...
        if (maskShapeTf == null)
        {
            ...
            // 원형 클리핑(Mask) 제거: 기존 Mask 컴포넌트를 추가하지 않습니다.
        }

        maskShapeTf = soulUI.transform.Find("FillViewport/FillMaskShape");
        if (maskShapeTf == null) return;

        // FillMaskShape가 이미 존재하는 경우에도 앵커/사이즈가 꼬였을 수 있으므로
        // 반드시 다시 Stretch/설정을 재적용합니다.
        {
        ...
        if (flowImg != null)
        {
            // FlowAnim(이미 존재할 때)도 RectTransform이 안 맞는 경우가 있어 재적용합니다.
    ...
    {
        // FillViewport의 높이 변화에 따라 Mask/Flow 영역 크기가 같이 흔들리지 않게
        // Mask(원형)과 채우기 이미지는 고정 정사각형으로 둡니다.
